...
global ch4
global div
set fid [open txt1/base.txt w]
for {set i 0} {$i<$val(nn)} {incr i} {
set ans [ran]
puts $fid $ans
}
close $fid
set fid [open "txt1/base.txt" r]
set file_data [read $fid]
set fid1 [open "txt1/$ch1.txt" w]
set fid2 [open "txt1/$ch2.txt" w]
set fid3 [open "txt1/$ch3.txt" w]
set fid4 [open "txt1/$ch4.txt" w]
set data [split $file_data "\n"]
set count 0
set first 0
...
close $fid4
close $fid
set fid1 [open "txt1/$ch1.txt" r]
set file_data1 [read $fid1]
set data1 [split $file_data1 "\n"]
set fid2 [open "txt1/$ch2.txt" r]
set file_data2 [read $fid2]
set data2 [split $file_data2 "\n"]
set fid3 [open "txt1/$ch3.txt" r]
set file_data3 [read $fid3]
set data3 [split $file_data3 "\n"]
set fid4 [open "txt1/$ch4.txt" r]
set file_data4 [read $fid4]
set data4 [split $file_data4 "\n"]
set count $first
foreach line $data1 {
if {$count!=$ch1} {
set fid_node [open "txt1/$count.txt" w]
puts $fid_node $line
close $fid_node
} else {
set fid_node [open "txt1/test.txt" w]
puts $fid_node $line
close $fid_node
}
set count [expr {$count+1}]
}
set fidc [open "txt1/test.txt" r]
set file_datac [read $fidc]
set fid_node [open "txt1/$ch1.txt" w]
...
set fidc [open "txt1/test.txt" r]
set file_datac [read $fidc]
set fid_node [open "txt1/$ch2.txt" w]
puts $fid_node $file_datac
close $fid_node
close $fidc
...
set fidc [open "txt1/test.txt" r]
set file_datac [read $fidc]
set fid_node [open "txt1/$ch3.txt" w]
puts $fid_node $file_datac
close $fid_node
close $fidc
...
set fidc [open "txt1/test.txt" r]
set file_datac [read $fidc]
set fid_node [open "txt1/$ch4.txt" w]
puts $fid_node $file_datac
close $fid_node
close $fidc
}
...
for {set j 0} {$j<$val(nn)} {incr j} {
for {set i 0} {$i<$val(nn)} {incr i} {
set connection($j,$i) 0
}
}
proc monitor {n1 n2 protocol service size} {
global connection
global first
global second
global third
...
global seventh
global eighth
global ch1 ch2 ch3 ch4 n
set x1 [expr int([$n($n1) set X_])]
set y1 [expr int([$n($n1) set Y_])]
set x2 [expr int([$n($n2) set X_])]
set y2 [expr int([$n($n2) set Y_])]
set df1 0
set df2 0
if {$n1>=$first && $n1<=$second} {
set x3 [expr int([$n($ch1) set X_])]
set y3 [expr int([$n($ch1) set Y_])]
set distx [expr {$x1-$x3}]
set distx [expr {$distx*$distx}]
set disty [expr {$y1-$y3}]
set disty [expr {$disty*$disty}]
set dist [expr {$distx+$disty}]
...
if {$n1>=$third && $n1<=$fourth} {
set x3 [expr int([$n($ch2) set X_])]
set y3 [expr int([$n($ch2) set Y_])]
set distx [expr {$x1-$x3}]
set distx [expr {$distx*$distx}]
set disty [expr {$y1-$y3}]
set disty [expr {$disty*$disty}]
...
if {$n1>=$fifth && $n1<=$sixth} {
set x3 [expr int([$n($ch3) set X_])]
set y3 [expr int([$n($ch3) set Y_])]
set distx [expr {$x1-$x3}]
set distx [expr {$distx*$distx}]
set disty [expr {$y1-$y3}]
set disty [expr {$disty*$disty}]
...
if {$n1>=$seventh && $n1<=$eighth} {
set x3 [expr int([$n($ch4) set X_])]
set y3 [expr int([$n($ch4) set Y_])]
set distx [expr {$x1-$x3}]
set distx [expr {$distx*$distx}]
set disty [expr {$y1-$y3}]
set disty [expr {$disty*$disty}]
...
set x3 [expr int([$n($ch1) set X_])]
set y3 [expr int([$n($ch1) set Y_])]
set distx [expr {$x2-$x3}]
set distx [expr {$distx*$distx}]
set disty [expr {$y2-$y3}]
set disty [expr {$disty*$disty}]
set dist [expr {$distx+$disty}]
set dist [expr { sqrt($dist)}]
puts "dist:$dist"
...
set leachval [sleach $n1]
set leachval2 [sleach $n2]
if {$leachval==1 && $leachval2==1 && $df1==1 && $df2==1} {
puts "monitor"
set fid [open "kdd/1.txt" r]
set file_data [read $fid]
set data [split $file_data "\n"]
set count 1
foreach line $data {
set data1 $line
#puts "$data1"
set fields [split $data1 ","]
set duration [lindex $fields 0]
set protocol1 [lindex $fields 1]
set service1 [lindex $fields 2]
set src_bytes [lindex $fields 4]
set dest_bytes [lindex $fields 5]
set land [lindex $fields 7]
set attacktyp [lindex $fields 41]
#puts "size:$size"
if {$protocol==$protocol1 && $service==$service1 && $src_bytes==$size && $land==$session($n1,$n2)} {
puts "$data1"
if {$attacktyp!="normal"} {
set now [$ns now]
puts "now:$now"
$ns at $now "$udp($n1) send 500 $n1:$n2:sstop"

}
}

#puts "$duration $protocol1 $service1"
#puts $fields
...
set connection($sender,$tes) [expr {$connection($sender,$tes)+1}]
puts "connection:$connection($sender,$tes)"
set now [$ns now]
#$ns at $now "monitor $sender $receiver $protocol $service $size"
...
set key1 [readvalue1 $sender $tes $receiver]
set now [$ns now]
$ns at 8.2 "$udpch2 send 500 $tes:$receiver:sstart"
if {$key1!=0} {
puts "key:$key1"
package require aes
set key1 [readvalue1 $sender $tes $receiver]
set fk 1
puts "key at udp1:$key1"
...
set encryptedData [aes::aes -dir encrypt -key $key1 $res]
set now1 [$ns now]
$ns at 8.3 "$udpch2 send 500 $tes:$receiver:$encryptedData:$sender"
puts "datatest:$encryptedData"
}

set now2 [$ns now]
$ns at 8.5 "$udpch2 send 500 $tes:$receiver:sstop"
} else {
puts "sender1:$sender1"
if {$fk==1} {
set key1 [readvalue1 $sender1 $sender $tes]
set fk 0
} else {
set key1 [readvalue $sender $tes]
}
puts "key at udp2:$key1"
set res 0
if {$key1!=0} {
set decrypt [aes::aes -dir decrypt -key $key1 $data]
set res [string trimright $decrypt \0]
...
$ns at $now "$udp($udpval) send 500 $n1:$n2:sstart"
...
$ns at $now "$udp($udpval) send 500 $n1:$n2:$encryptedData"
$ns at $now "$udp($udpval) send 500 $n1:$n2:sstop"
}
...
#$ns at 4.0 keygen
#$ns at 4.0 "test 0 1 0"
#$ns at 4.0 "test 5 15 5"
#$ns at 4.1 keygen
#$ns at 4.2 "test 5 15 5"
#$ns at 4.5 keygen
#$ns at 4.6 "test 5 15 5"
#$ns at 5.0 keygen
#$ns at 5.2 "test 5 15 5"
#$ns at 5.5 keygen
#$ns at 5.7 "test 5 15 5"
#$ns at 6.0 keygen
#$ns at 6.2 "test 5 15 5"
#==========================================================================================
